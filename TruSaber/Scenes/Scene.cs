...
namespace TruSaber.Scenes
{
    public abstract class Scene : IScene
    {
        ...
        private SortingFilteringCollection<IDrawable> _drawables = new SortingFilteringCollection<IDrawable>(
            d => d.Visible, (d, handler) => d.VisibleChanged += handler, (d, handler) => d.VisibleChanged -= handler,
        ...
        private SortingFilteringCollection<IUpdateable> _updateables = new SortingFilteringCollection<IUpdateable>(
            u => u.Enabled, (u, handler) => u.EnabledChanged += handler, (u, handler) => u.EnabledChanged -= handler,
        ...
        private static readonly Action<IDrawable, GameTime> DrawAction =
            (Action<IDrawable, GameTime>) ((drawable, gameTime) => drawable.Draw(gameTime));

        private static readonly Action<IUpdateable, GameTime> UpdateAction =
            (Action<IUpdateable, GameTime>) ((updateable, gameTime) => updateable.Update(gameTime));
        ...
        private void InitializeExistingComponents()
        {
            for (int index = 0; index < this.Components.Count; ++index)
                this.Components[index].Initialize();
        }
