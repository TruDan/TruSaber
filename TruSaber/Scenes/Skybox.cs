...
{
    public class Skybox : DrawableEntity
    {
        ...
        public Skybox(IGame game) : base(game)
        {
        ...
        protected override void LoadContent()
        {
            ...
            texture = Game.Content.Load<Texture2D>("Textures/Skybox/Spacebox");
        ...
        public void CreateOrSetupSkyBoxOrSphere(out VertexPositionNormalColorUv[] vertices_sborss, out int[] indices_sborss, bool createSphere, int numberOfTesselations, float scaleDistance, bool skyBoxUseImageCrossType, bool skyBoxCrossImageLeft)
        {
            ...
            {
                sbt.PrivateCreateSkyboxFromSixBlockImage(out vertices_sborss, out indices_sborss, scaleDistance, false,
                    0, 1, 5, 4, 3, 2);
            }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            {
                pass.Apply();
                GraphicsDevice.DrawUserIndexedPrimitives(PrimitiveType.TriangleList, vertices_skybox, 0, 2, indices_skybox, 0, indices_skybox.Length / 3, VertexPositionNormalColorUv.VertexDeclaration);
            }
        }
    }
    ...
    public class SkyBoxTesselator
    {
        ...
        )
        {
            ...
            int i = 0;
            // front;
            vertices_sborss[i].Position = p[0];
            i++; //left
            vertices_sborss[i].Position = p[1];
            i++;
            vertices_sborss[i].Position = p[2];
            i++; //right
            vertices_sborss[i].Position = p[3];
            i++;
            // back 
            vertices_sborss[i].Position = p[6];
            i++;
            vertices_sborss[i].Position = p[7];
            i++;
            vertices_sborss[i].Position = p[4];
            i++;
            vertices_sborss[i].Position = p[5];
            i++;
            // right
            vertices_sborss[i].Position = p[2];
            i++;
            ...
            i = 0;
            int k = 0; // front
            indices_sborss[i] = k + 0;
            i++;
            indices_sborss[i] = k + 1;
            i++;
            indices_sborss[i] = k + 2;
            i++;
            indices_sborss[i] = k + 2;
            i++;
            indices_sborss[i] = k + 1;
            i++;
            indices_sborss[i] = k + 3;
            i++;
        ...
        {
            Subdivide(source_vertices, source_indices, Vector3.Zero, distance, numberOfSubdivisions, out dest_vertices,
                out dest_indices);
        }
        ...
        )
        {
            ...
            for (int f = 0; f < Faces; f++)
            {
                ...
                for (int j = 1; j < d_verticesPerDirection - 1; j++)
                {
                    ...
                    float lerptime =
                        (float) (j) / (float) (d_verticesPerDirection - 1); //j / (d_verticesPerDirection - d_rt_Edge);
                ...
                for (int down = 0; down < d_verticesPerDirection; down++)
                {
                    ...
                    for (int across = 1; across < d_verticesPerDirection - 1; across++)
                    {
                        ...
                        dv[creationindex] =
                            LerpSubDivisionVeritces(dv[fromLeft], dv[toRight], lerptime, distance, origin);
                    }
                }
            }
            ...
            for (int f = 0; f < Faces; f++)
            {
                ...
                for (int across = 0; across < d_verticesPerDirection - 1; across++)
                {
                    ...
                    for (int down = 0; down < d_verticesPerDirection - 1; down++)
                    {
                        ...
                        // set the indices
                        di[dIndice_Index] = vindex0;
                        dIndice_Index++;
                        di[dIndice_Index] = vindex1;
                        dIndice_Index++;
                        di[dIndice_Index] = vindex2;
                        dIndice_Index++;
        ...
        )
        {
            ...
            B.TextureCoordinateA = from.TextureCoordinateA +
                                   ((to.TextureCoordinateA - from.TextureCoordinateA) * lerpPercentage);
            return B;
        }
    }
    ...
    public struct VertexPositionNormalColorUv : IVertexType
    {
        ...
        (
            new VertexElement(VertexElementByteOffset.PositionStartOffset(), VertexElementFormat.Vector3,
                VertexElementUsage.Position, 0),
            new VertexElement(VertexElementByteOffset.OffsetColor(), VertexElementFormat.Color,
                VertexElementUsage.Color,
                0),
            new VertexElement(VertexElementByteOffset.OffsetVector3(), VertexElementFormat.Vector3,
                VertexElementUsage.Normal, 0),
            new VertexElement(VertexElementByteOffset.OffsetVector2(), VertexElementFormat.Vector2,
    ...
    public struct VertexElementByteOffset
    {
        ...
        {
            currentByteSize = 0;
            var s = sizeof(float) * 3;
            currentByteSize += s;
            return currentByteSize - s;
        }
