...
namespace RocketUI.Serialization
{
	public class PropertyStore : Dictionary<object, object>
	{
		...
		public void TriggerEvent<T>(object key, object sender, T args)
			where T : EventArgs
		{
			if (TryGetValue(key, out var existingDelegate) && existingDelegate != null)
			{
...
#if PCL
		public bool Set<T>(object key, T value, PropertyChangedEventHandler propertyChanged, T defaultValue =
 default(T), [CallerMemberName] string propertyName = null)
#else
		public bool Set<T>(object key, T value, PropertyChangedEventHandler propertyChanged, T defaultValue,
						   string propertyName)
#endif
		...
		{
			readonly Action<EventHandler<EventArgs>> _removeExecute;
			readonly Action<bool>                    _setEnabled;
			readonly Func<object>                    _getArgument;
			public   ICommand                        Command { get; set; }

			public CommandWrapper(ICommand                        command, Action<bool> setEnabled,
								  Action<EventHandler<EventArgs>> addExecute,
								  Action<EventHandler<EventArgs>> removeExecute, Func<object> getParameter)
			{
				this.Command       = command;
				this._setEnabled    = setEnabled;
				this._removeExecute = removeExecute;
				this._getArgument   = getParameter;
			...
			{
				_removeExecute(Command_Execute);
				Command.CanExecuteChanged -= Command_CanExecuteChanged;
			}
			...
			object Parameter
			{
				get { return _getArgument != null ? _getArgument() : null; }
			}

			public void SetEnabled()
			{
				if (_setEnabled != null)
					_setEnabled(Command.CanExecute(Parameter));
			}
		}
		...
		public void SetCommand(object                          key, ICommand value, Action<bool> setEnabled,
							   Action<EventHandler<EventArgs>> addExecute,
							   Action<EventHandler<EventArgs>> removeExecute, Func<object> getParameter)
		{
			...
			Set(key,
				value != null ? new CommandWrapper(value, setEnabled, addExecute, removeExecute, getParameter) : null);
		}
